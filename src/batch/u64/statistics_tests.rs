...
mod unit_tests {
    ...
    #[inline]
    fn iv(start: u64, end_excl: u64) -> U64CO {
    ...
    mod interval_count_tests {
        ...
        #[test]
        fn empty_set() {
            let set = U64COBatchSet::default();
            assert_eq!(set.interval_count(), 0);
        }

        #[test]
        fn single_interval() {
            let set = U64COBatchSet::from([iv(3, 7)]);
            assert_eq!(set.interval_count(), 1);
        }

        #[test]
        fn multi_interval() {
            let set = U64COBatchSet::from([iv(1, 3), iv(5, 8), iv(10, 11)]);
            assert_eq!(set.interval_count(), 3);
        }

        #[test]
        fn normalized_count_after_merge() {
            let set = U64COBatchSet::from([iv(5, 7), iv(1, 3), iv(3, 5), iv(10, 12)]);
    ...
    mod coverage_len_of_tests {
        ...
        fn query_before_all_intervals() {
            let set = U64COBatchSet::from([iv(5, 8), iv(10, 12)]);
            assert_eq!(set.coverage_len_of(iv(0, 4)), 0);
        }

        #[test]
        fn query_after_all_intervals() {
            let set = U64COBatchSet::from([iv(1, 3), iv(5, 8)]);
            assert_eq!(set.coverage_len_of(iv(10, 20)), 0);
        }

        #[test]
        fn single_interval_basic() {
            let set = U64COBatchSet::from([iv(5, 10)]);
        ...
        fn multi_interval_with_gaps() {
            let set = U64COBatchSet::from([iv(1, 3), iv(5, 7), iv(9, 11)]);
        ...
        fn query_spans_multiple_segments() {
            let set = U64COBatchSet::from([iv(2, 4), iv(7, 9), iv(12, 15)]);
        ...
        fn matches_pointwise_semantics() {
            let set = U64COBatchSet::from([iv(1, 4), iv(6, 9), iv(20, 30)]);
            let queries = [
                iv(0, 1),
                iv(0, 2),
                iv(3, 6),
            ...
            for q in queries {
                let expect = q.iter().filter(|&x| set.contains_point(x)).count() as u64;
    ...
    mod coverage_ratio_of_tests {
        ...
        fn fully_covered_query() {
            let set = U64COBatchSet::from([iv(3, 9)]);
    ...
    mod normalized_statistics_tests {
        ...
        fn normalized_set_statistics_are_consistent() {
            let set = U64COBatchSet::from([iv(8, 10), iv(1, 3), iv(3, 5), iv(5, 8), iv(20, 21)]);
