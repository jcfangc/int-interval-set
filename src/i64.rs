...
/// Read-only canonical interval set for `I64CO`.
///
/// A `I64COSet` stores a normalized collection of half-open `I64CO` intervals:
///
...
///
/// Construction is intentionally restricted. Use `I64COSetBuilder` for normal
/// construction.
pub mod set {
    use super::*;
    ...
    mod basic {
        ...
        impl I64COSet {
            ...
            #[inline]
            pub(in crate::i64) unsafe fn new_unchecked(intervals: Vec<I64CO>) -> Self {
            ...
            #[inline]
            fn is_canonical(intervals: &[I64CO]) -> bool {
        ...
        impl I64COSet {
            ...
            ///
            /// For the `I64CO` domain, the maximum canonical interval count is 128,
            /// e.g. alternating single-point intervals across `[i64::MIN, i64::MAX)`.
            #[inline]
    ...
    mod predicates {
        ...
        impl I64COSet {
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn contains_interval(&self, query: I64CO) -> bool {
                let i = self
                    .intervals
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn intersects_interval(&self, query: I64CO) -> bool {
                let i = self
                    .intervals
    ...
    mod search {
        ...
        mod interval {
            ...
            impl I64COSet {
                ...
                /// returned intervals.
                #[inline]
                pub fn intervals_intersecting(&self, query: I64CO) -> impl Iterator<Item = I64CO> {
                    let i = self
                        .intervals
                        .partition_point(|iv| iv.end_excl() <= query.start());
                ...
                #[inline]
                pub fn intersections(&self, query: I64CO) -> impl Iterator<Item = I64CO> {
    ...
    mod coverage {
        ...
        impl I64COSet {
            ...
            ///
            /// Since `I64COSet` is canonical, all intersection segments are
            /// disjoint, so summing their lengths is valid.
            ///
            /// The result is always `<= query.len()`.
            #[inline]
            pub fn covered_len(&self, query: I64CO) -> u64 {
                self.intersections(query).map(|iv| iv.len()).sum()
            }

            /// Returns the uncovered length inside `query`.
            #[inline]
            pub fn uncovered_len(&self, query: I64CO) -> u64 {
                query.len() - self.covered_len(query)
            }

            /// Returns `covered_len(query) / query.len()` as `f32`.
            ///
            /// `query.len()` is non-zero because `I64CO` cannot represent an
            /// empty interval.
            #[inline]
            pub fn coverage_ratio(&self, query: I64CO) -> f32 {
...
/// Concurrent builder for `I64COSet`.
///
...
pub mod builder {
    ...
    use super::set::I64COSet;
    use super::*;

    /// Private ordering key for storing `I64CO` inside `SkipSet`.
    ///
    /// `I64CO` does not need to implement `Ord` in its own crate. This wrapper
    ...
    /// Concurrent write-side builder for `I64COSet`.
    ///
    ...
    pub struct I64COSetBuilder {
        raw: SkipSet<Key>,
    }

    impl I64COSetBuilder {
        ...
        pub fn seal(self) -> I64COSet {
            ...
            // intervals.
            unsafe { I64COSet::new_unchecked(out) }
        }
    }
}
