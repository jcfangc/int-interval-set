...
use crate::{
    I32COSetBuilder,
    i32::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = I32COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(i32::MIN, i32::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(i32::MAX - 1, i32::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(i32::MIN, i32::MIN + 1), iv(2, 3), iv(i32::MAX - 1, i32::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(i32, i32)>) -> Vec<I32CO> {
    xs.sort_unstable();

    let mut out: Vec<I32CO> = Vec::new();
