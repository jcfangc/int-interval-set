...
fn returns_none_on_empty_set() {
    ...
    assert_eq!(set.interval_containing_point(i32::MIN), None);
    assert_eq!(set.interval_containing_point(i32::MAX / 2), None);
    assert_eq!(set.interval_containing_point(i32::MAX), None);
}
...
fn handles_domain_edges() {
    let set = build([(i32::MIN, i32::MIN + 1), (i32::MAX - 1, i32::MAX)]);

    assert_eq!(
        set.interval_containing_point(i32::MIN),
        Some(iv(i32::MIN, i32::MIN + 1))
    );
    assert_eq!(set.interval_containing_point(i32::MIN + 1), None);

    assert_eq!(
        set.interval_containing_point(i32::MAX - 1),
        Some(iv(i32::MAX - 1, i32::MAX))
    );
    assert_eq!(set.interval_containing_point(i32::MAX), None);
}
...
fn representative_points_match_contains_point_predicate() {
    ...
    for x in [i32::MIN, 9, 10, 19, 20, 30, 39, 40, 55, 60, i32::MAX] {
        assert_eq!(
...
proptest! {
    ...
    fn prop_interval_containing_point_matches_slice_find(
        ...
        let set: I32COSet = xs.into_iter().collect();
