...
use crate::{
    I16COSet,
    i16::test_support::{arb_iv, build, iv},
};
...
fn returns_none_on_empty_set() {
    ...
    assert_eq!(set.interval_containing_point(i16::MIN), None);
    assert_eq!(set.interval_containing_point(i16::MAX / 2), None);
    assert_eq!(set.interval_containing_point(i16::MAX), None);
}
...
fn handles_domain_edges() {
    let set = build([(i16::MIN, i16::MIN + 1), (i16::MAX - 1, i16::MAX)]);

    assert_eq!(
        set.interval_containing_point(i16::MIN),
        Some(iv(i16::MIN, i16::MIN + 1))
    );
    assert_eq!(set.interval_containing_point(i16::MIN + 1), None);

    assert_eq!(
        set.interval_containing_point(i16::MAX - 1),
        Some(iv(i16::MAX - 1, i16::MAX))
    );
    assert_eq!(set.interval_containing_point(i16::MAX), None);
}
...
fn representative_points_match_contains_point_predicate() {
    ...
    for x in [i16::MIN, 9, 10, 19, 20, 30, 39, 40, 55, 60, i16::MAX] {
        assert_eq!(
...
proptest! {
    ...
    fn prop_interval_containing_point_matches_slice_find(
        ...
        let set: I16COSet = xs.into_iter().collect();
