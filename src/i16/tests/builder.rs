...
use crate::{
    I16COSetBuilder,
    i16::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = I16COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(i16::MIN, i16::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(i16::MAX - 1, i16::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(i16::MIN, i16::MIN + 1), iv(2, 3), iv(i16::MAX - 1, i16::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(i16, i16)>) -> Vec<I16CO> {
    xs.sort_unstable();

    let mut out: Vec<I16CO> = Vec::new();
