...
impl I16COSet {
    ...
    #[inline]
    fn contiguous_range_with_interval(&self, query: I16CO) -> std::ops::Range<usize> {
        let start = self
            .intervals
    ...
    #[inline]
    pub fn intersection_with_interval(&self, query: I16CO) -> Self {
        let intervals = self
    ...
    #[inline]
    pub fn union_with_interval(&self, query: I16CO) -> Self {
    ...
    #[inline]
    pub fn difference_with_interval(&self, query: I16CO) -> Self {
        let hit_start = self
            .intervals
        ...
        let left = I16CO::try_new(first.start(), query.start());
        let right = I16CO::try_new(query.end_excl(), last.end_excl());
    ...
    #[inline]
    pub fn symmetric_difference_with_interval(&self, query: I16CO) -> Self {
        let range = self.contiguous_range_with_interval(query);
        ...
        for source in self.intervals[range.clone()].iter().copied() {
            ...
            if let Some(residual) = I16CO::try_new(cursor, overlap.start()) {
        ...
        if let Some(residual) = I16CO::try_new(cursor, union_component.end_excl()) {
            intervals.push(residual);
        }
