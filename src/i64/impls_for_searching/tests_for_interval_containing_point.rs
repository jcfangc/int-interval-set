...
use crate::{
    I64COSet,
    i64::test_support::{arb_iv, build, iv},
};
...
fn returns_none_on_empty_set() {
    ...
    assert_eq!(set.interval_containing_point(i64::MIN), None);
    assert_eq!(set.interval_containing_point(i64::MAX / 2), None);
    assert_eq!(set.interval_containing_point(i64::MAX), None);
}
...
fn handles_domain_edges() {
    ...
    assert_eq!(
        set.interval_containing_point(i64::MIN),
        Some(iv(i64::MIN, i64::MIN + 1))
    );
    assert_eq!(set.interval_containing_point(i64::MIN + 1), None);

    assert_eq!(
        set.interval_containing_point(i64::MAX - 1),
        Some(iv(i64::MAX - 1, i64::MAX))
    );
    assert_eq!(set.interval_containing_point(i64::MAX), None);
}
...
fn representative_points_match_contains_point_predicate() {
    ...
    for x in [i64::MIN, 9, 10, 19, 20, 30, 39, 40, 55, 60, i64::MAX] {
        assert_eq!(
...
proptest! {
    ...
    fn prop_interval_containing_point_matches_slice_find(
        ...
        let set: I64COSet = xs.into_iter().collect();
