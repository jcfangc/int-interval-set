...
use crate::{
    I64COSetBuilder,
    i64::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = I64COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(i64::MIN, i64::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(i64::MAX - 1, i64::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(i64::MIN, i64::MIN + 1), iv(2, 3), iv(i64::MAX - 1, i64::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(i64, i64)>) -> Vec<I64CO> {
    xs.sort_unstable();

    let mut out: Vec<I64CO> = Vec::new();
