...
use crate::i128::test_support::{intervals, iv};

use super::*;

#[test]
fn from_par_iter_empty() {
    let set: I128COSet = Vec::<I128CO>::new().into_par_iter().collect();
...
fn from_par_iter_merges_many_reduced_runs() {
    ...
    let set: I128COSet = input.into_par_iter().collect();
...
fn from_par_iter_matches_from_iter_for_large_input() {
    let input: Vec<I128CO> = (0..BATCH_SIZE * 16)
        .map(|i| match i % 8 {
            0 => iv(40, 45),
            1 => iv(0, 2),
            2 => iv(10, 12),
    ...
    let sequential: I128COSet = input.iter().copied().collect();
    let parallel: I128COSet = input.into_par_iter().collect();
