...
/// Read-only canonical interval set for `U128CO`.
///
/// A `U128COSet` stores a normalized collection of half-open `U128CO` intervals:
///
...
///
/// Construction is intentionally restricted. Use `U128COSetBuilder` for normal
/// construction.
pub mod set {
    use super::*;
    ...
    mod basic {
        ...
        impl U128COSet {
            ...
            #[inline]
            pub(in crate::u128) unsafe fn new_unchecked(intervals: Vec<U128CO>) -> Self {
            ...
            #[inline]
            fn is_canonical(intervals: &[U128CO]) -> bool {
        ...
        impl U128COSet {
            ...
            ///
            /// For the `U128CO` domain, the maximum canonical interval count is
            ...
            /// adjacent intervals.
            #[inline]
            pub fn as_slice(&self) -> &[U128CO] {
                &self.intervals
            }

            /// Iterates over canonical intervals by value.
            #[inline]
            pub fn iter_intervals(&self) -> impl Iterator<Item = U128CO> {
    ...
    mod predicates {
        ...
        impl U128COSet {
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn contains_point(&self, x: u128) -> bool {
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn contains_interval(&self, query: U128CO) -> bool {
                let i = self
                    .intervals
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn intersects_interval(&self, query: U128CO) -> bool {
                let i = self
                    .intervals
    ...
    mod search {
        ...
        mod point {
            ...
            impl U128COSet {
                ...
                /// Complexity: `O(log n)`.
                #[inline]
                pub fn interval_containing_point(&self, x: u128) -> Option<U128CO> {
                    let i = self.intervals.partition_point(|iv| iv.start() <= x);
        ...
        mod interval {
            ...
            impl U128COSet {
                ...
                /// returned intervals.
                #[inline]
                pub fn intervals_intersecting(&self, query: U128CO) -> impl Iterator<Item = U128CO> {
                    let i = self
                        .intervals
                        .partition_point(|iv| iv.end_excl() <= query.start());
                ...
                #[inline]
                pub fn intersections(&self, query: U128CO) -> impl Iterator<Item = U128CO> {
    ...
    mod coverage {
        ...
        impl U128COSet {
            ...
            ///
            /// Since `U128COSet` is canonical, all intersection segments are
            /// disjoint, so summing their lengths is valid.
            ///
            /// The result is always `<= query.len()`.
            #[inline]
            pub fn covered_len(&self, query: U128CO) -> u128 {
                self.intersections(query).map(|iv| iv.len()).sum()
            }

            /// Returns the uncovered length inside `query`.
            #[inline]
            pub fn uncovered_len(&self, query: U128CO) -> u128 {
                query.len() - self.covered_len(query)
            }

            /// Returns `covered_len(query) / query.len()` as `f32`.
            ///
            /// `query.len()` is non-zero because `U128CO` cannot represent an
            /// empty interval.
            #[inline]
            pub fn coverage_ratio(&self, query: U128CO) -> f32 {
...
/// Concurrent builder for `U128COSet`.
///
...
pub mod builder {
    ...
    use super::set::U128COSet;
    use super::*;

    /// Private ordering key for storing `U128CO` inside `SkipSet`.
    ///
    /// `U128CO` does not need to implement `Ord` in its own crate. This wrapper
    ...
    impl U128COSetBuilder {
        ...
        pub fn seal(self) -> U128COSet {
            ...
            let Some(mut cur) = iter.next() else {
                ...
                return unsafe { U128COSet::new_unchecked(Vec::new()) };
            };
            ...
            // intervals.
            unsafe { U128COSet::new_unchecked(out) }
        }
    }
}
