...
use crate::{
    UsizeCOSetBuilder,
    usize::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = UsizeCOSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(usize::MIN, usize::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(usize::MAX - 1, usize::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(usize::MIN, usize::MIN + 1), iv(2, 3), iv(usize::MAX - 1, usize::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(usize, usize)>) -> Vec<UsizeCO> {
    xs.sort_unstable();

    let mut out: Vec<int_interval::UsizeCO> = Vec::new();
