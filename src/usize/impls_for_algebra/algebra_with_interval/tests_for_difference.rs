...
fn difference_handles_domain_edges() {
    let set = build([(usize::MIN, usize::MIN + 10), (usize::MAX - 10, usize::MAX)]);

    assert_eq!(
        set.difference_with_interval(iv(usize::MIN, usize::MIN + 5))
            .as_slice(),
        &[iv(usize::MIN + 5, usize::MIN + 10), iv(usize::MAX - 10, usize::MAX)]
    );

    assert_eq!(
        set.difference_with_interval(iv(usize::MAX - 5, usize::MAX))
            .as_slice(),
        &[iv(usize::MIN, usize::MIN + 10), iv(usize::MAX - 10, usize::MAX - 5)]
    );
}
