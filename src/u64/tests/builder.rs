...
use crate::{
    U64COSetBuilder,
    u64::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = U64COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(u64::MIN, u64::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(u64::MAX - 1, u64::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(u64::MIN, u64::MIN + 1), iv(2, 3), iv(u64::MAX - 1, u64::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(u64, u64)>) -> Vec<U64CO> {
    xs.sort_unstable();

    let mut out: Vec<int_interval::U64CO> = Vec::new();
