...
use crate::u64::test_support::{intervals, iv};

use super::*;

#[test]
fn from_par_iter_empty() {
    let set: U64COSet = Vec::<U64CO>::new().into_par_iter().collect();
...
fn from_par_iter_merges_many_reduced_runs() {
    ...
    let set: U64COSet = input.into_par_iter().collect();
...
fn from_par_iter_matches_from_iter_for_large_input() {
    let input: Vec<U64CO> = (0..BATCH_SIZE * 16)
        .map(|i| match i % 8 {
            0 => iv(40, 45),
            1 => iv(0, 2),
            2 => iv(10, 12),
    ...
    let sequential: U64COSet = input.iter().copied().collect();
    let parallel: U64COSet = input.into_par_iter().collect();
