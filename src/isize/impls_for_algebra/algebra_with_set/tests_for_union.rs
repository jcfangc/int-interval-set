...
fn union_handles_domain_edges() {
    let left = build([(isize::MIN, isize::MIN + 10), (isize::MAX - 10, isize::MAX)]);
    let right = build([(isize::MIN + 10, isize::MAX - 10)]);

    assert_eq!(
        left.union_with_set(&right).as_slice(),
        &[iv(isize::MIN, isize::MAX)]
    );
}
...
proptest! {
    ...
    fn prop_union_matches_pointwise_membership(
        ...
        let left: IsizeCOSet = xs.into_iter().collect();
        let right: IsizeCOSet = ys.into_iter().collect();
