...
fn contains_point_works_across_multiple_intervals() {
    ...
    assert!(!set.contains_point(u32::MAX));
}
...
fn contains_point_works_on_empty_set() {
    ...
    assert!(!set.contains_point(u32::MIN));
    assert!(!set.contains_point(u32::MAX / 2));
    assert!(!set.contains_point(u32::MAX));
}
...
fn contains_interval_works_on_empty_set() {
    ...
    assert!(!set.contains_interval(iv(u32::MAX - 1, u32::MAX)));
}
...
fn intersects_interval_works_on_empty_set() {
    ...
    assert!(!set.intersects_interval(iv(u32::MAX - 1, u32::MAX)));
}
...
fn predicates_handle_domain_edges() {
    ...
    assert!(set.contains_point(u32::MIN));
    assert!(!set.contains_point(u32::MIN + 1));

    assert!(set.contains_point(u32::MAX - 1));
    assert!(!set.contains_point(u32::MAX));

    assert!(set.contains_interval(iv(u32::MIN, u32::MIN + 1)));
    assert!(set.contains_interval(iv(u32::MAX - 1, u32::MAX)));

    assert!(set.intersects_interval(iv(u32::MIN, u32::MIN + 1)));
    assert!(set.intersects_interval(iv(u32::MAX - 1, u32::MAX)));
}
