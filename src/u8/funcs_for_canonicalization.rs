...
#[inline]
pub(super) fn is_canonical(intervals: &[U8CO]) -> bool {
    intervals.windows(2).all(|w| w[0].end_excl() < w[1].start())
}

#[inline]
pub(super) fn normalize(mut intervals: Vec<U8CO>) -> Vec<U8CO> {
    intervals.sort_unstable();
    canonicalize_sorted(intervals)
}

#[inline]
pub(super) fn merge(left: Vec<U8CO>, right: Vec<U8CO>) -> Vec<U8CO> {
    if left.is_empty() {
        return right;
    }
...
pub(super) fn canonicalize_sorted<I>(intervals: I) -> Vec<U8CO>
where
...
pub(super) fn merge_sorted<I, J>(left: I, right: J) -> impl Iterator<Item = U8CO>
where
    I: IntoIterator<Item = U8CO>,
