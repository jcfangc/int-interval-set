...
fn predicates_contains_interval_works_on_empty_set() {
    ...
    assert!(!set.contains_interval(iv(u8::MAX - 1, u8::MAX)));
}
...
fn predicates_handle_domain_edges() {
    ...
    assert!(set.contains_point(u8::MIN));
    assert!(!set.contains_point(u8::MIN + 1));

    assert!(set.contains_point(u8::MAX - 1));
    assert!(!set.contains_point(u8::MAX));

    assert!(set.contains_interval(iv(u8::MIN, u8::MIN + 1)));
    assert!(set.contains_interval(iv(u8::MAX - 1, u8::MAX)));

    assert!(set.intersects_interval(iv(u8::MIN, u8::MIN + 1)));
    assert!(set.intersects_interval(iv(u8::MAX - 1, u8::MAX)));
}
