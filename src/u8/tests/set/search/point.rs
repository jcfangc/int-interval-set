...
fn search_point_handles_domain_edges() {
    ...
    assert_eq!(
        set.interval_containing_point(u8::MIN),
        Some(iv(u8::MIN, u8::MIN + 1))
    );
    assert_eq!(set.interval_containing_point(u8::MIN + 1), None);

    assert_eq!(
        set.interval_containing_point(u8::MAX - 1),
        Some(iv(u8::MAX - 1, u8::MAX))
    );
    assert_eq!(set.interval_containing_point(u8::MAX), None);
}
