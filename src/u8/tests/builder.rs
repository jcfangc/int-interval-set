use crate::{
    U8COSetBuilder,
    u8::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = U8COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(u8::MIN, u8::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(u8::MAX - 1, u8::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(u8::MIN, u8::MIN + 1), iv(2, 3), iv(u8::MAX - 1, u8::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(u8, u8)>) -> Vec<U8CO> {
    xs.sort_unstable();

    let mut out: Vec<int_interval::U8CO> = Vec::new();
