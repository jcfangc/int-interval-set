...
fn difference_handles_domain_edges() {
    let set = build([(u8::MIN, u8::MIN + 10), (u8::MAX - 10, u8::MAX)]);

    assert_eq!(
        set.difference_with_interval(iv(u8::MIN, u8::MIN + 5))
            .as_slice(),
        &[iv(u8::MIN + 5, u8::MIN + 10), iv(u8::MAX - 10, u8::MAX)]
    );

    assert_eq!(
        set.difference_with_interval(iv(u8::MAX - 5, u8::MAX))
            .as_slice(),
        &[iv(u8::MIN, u8::MIN + 10), iv(u8::MAX - 10, u8::MAX - 5)]
    );
}
