use crate::{
    I8COSetBuilder,
    i8::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = I8COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(i8::MIN, i8::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(i8::MAX - 1, i8::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(i8::MIN, i8::MIN + 1), iv(2, 3), iv(i8::MAX - 1, i8::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(i8, i8)>) -> Vec<I8CO> {
    xs.sort_unstable();

    let mut out: Vec<I8CO> = Vec::new();
