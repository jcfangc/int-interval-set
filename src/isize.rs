...
/// Read-only canonical interval set for `IsizeCO`.
///
/// A `IsizeCOSet` stores a normalized collection of half-open `IsizeCO` intervals:
///
...
///
/// Construction is intentionally restricted. Use `IsizeCOSetBuilder` for normal
/// construction.
pub mod set {
    use super::*;
    ...
    pub struct IsizeCOSet {
        intervals: Arc<[IsizeCO]>,
    }
    ...
    mod basic {
        ...
        impl IsizeCOSet {
            ...
            #[inline]
            pub(in crate::isize) unsafe fn new_unchecked(intervals: Vec<IsizeCO>) -> Self {
            ...
            ///
            /// `IsizeCO` itself already guarantees single-interval validity. This
            /// function only checks the relationship between adjacent intervals.
            #[inline]
            fn is_canonical(intervals: &[IsizeCO]) -> bool {
...
/// Concurrent builder for `IsizeCOSet`.
///
...
/// immutable `IsizeCOSet`.
pub mod builder {
    use std::cmp::Ordering;

    use crossbeam_skiplist::SkipSet;

    use super::set::IsizeCOSet;
    use super::*;

    /// Private ordering key for storing `IsizeCO` inside `SkipSet`.
    ///
    /// `IsizeCO` does not need to implement `Ord` in its own crate. This wrapper
    ...
    /// Concurrent write-side builder for `IsizeCOSet`.
    ///
    ...
    pub struct IsizeCOSetBuilder {
        raw: SkipSet<Key>,
    }

    impl IsizeCOSetBuilder {
        ...
        pub fn seal(self) -> IsizeCOSet {
            ...
            // intervals.
            unsafe { IsizeCOSet::new_unchecked(out) }
        }
    }
}
