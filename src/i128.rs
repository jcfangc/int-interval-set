...
/// Read-only canonical interval set for `I128CO`.
///
/// A `I128COSet` stores a normalized collection of half-open `I128CO` intervals:
///
...
///
/// Construction is intentionally restricted. Use `I128COSetBuilder` for normal
/// construction.
pub mod set {
    use super::*;
    ...
    ///
    /// Internally this is an `Arc<[I128CO]>`, so cloning a `I128COSet` is cheap.
    ///
    /// Canonical invariant:
    ///
    /// ```text
    ...
    pub struct I128COSet {
        intervals: Arc<[I128CO]>,
    }
    ...
    mod basic {
        ...
        impl I128COSet {
            ...
            #[inline]
            pub(in crate::i128) unsafe fn new_unchecked(intervals: Vec<I128CO>) -> Self {
            ...
            #[inline]
            fn is_canonical(intervals: &[I128CO]) -> bool {
        ...
        impl I128COSet {
            ...
            ///
            /// For the `I128CO` domain, the maximum canonical interval count is 128,
            /// e.g. alternating single-point intervals across `[i128::MIN, i128::MAX)`.
            #[inline]
            pub fn interval_count(&self) -> u128 {
                self.intervals.len() as u128
            }
    ...
    mod predicates {
        ...
        impl I128COSet {
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn contains_interval(&self, query: I128CO) -> bool {
                let i = self
                    .intervals
            ...
            /// Complexity: `O(log n)`.
            #[inline]
            pub fn intersects_interval(&self, query: I128CO) -> bool {
                let i = self
                    .intervals
    ...
    mod search {
        ...
        mod interval {
            ...
            impl I128COSet {
                ...
                /// returned intervals.
                #[inline]
                pub fn intervals_intersecting(&self, query: I128CO) -> impl Iterator<Item = I128CO> {
                    let i = self
                        .intervals
                        .partition_point(|iv| iv.end_excl() <= query.start());
                ...
                #[inline]
                pub fn intersections(&self, query: I128CO) -> impl Iterator<Item = I128CO> {
    ...
    mod coverage {
        ...
        impl I128COSet {
            ...
            ///
            /// Since `I128COSet` is canonical, all intersection segments are
            /// disjoint, so summing their lengths is valid.
            ///
            /// The result is always `<= query.len()`.
            #[inline]
            pub fn covered_len(&self, query: I128CO) -> u128 {
                self.intersections(query).map(|iv| iv.len()).sum()
            }

            /// Returns the uncovered length inside `query`.
            #[inline]
            pub fn uncovered_len(&self, query: I128CO) -> u128 {
                query.len() - self.covered_len(query)
            }

            /// Returns `covered_len(query) / query.len()` as `f32`.
            ///
            /// `query.len()` is non-zero because `I128CO` cannot represent an
            /// empty interval.
            #[inline]
            pub fn coverage_ratio(&self, query: I128CO) -> f32 {
...
/// Concurrent builder for `I128COSet`.
///
...
pub mod builder {
    ...
    use super::set::I128COSet;
    use super::*;

    /// Private ordering key for storing `I128CO` inside `SkipSet`.
    ///
    /// `I128CO` does not need to implement `Ord` in its own crate. This wrapper
    ...
    /// Concurrent write-side builder for `I128COSet`.
    ///
    ...
    pub struct I128COSetBuilder {
        raw: SkipSet<Key>,
    }

    impl I128COSetBuilder {
        ...
        pub fn seal(self) -> I128COSet {
            ...
            // intervals.
            unsafe { I128COSet::new_unchecked(out) }
        }
    }
}
