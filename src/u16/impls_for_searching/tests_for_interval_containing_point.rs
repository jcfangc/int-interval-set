...
fn returns_none_on_empty_set() {
    ...
    assert_eq!(set.interval_containing_point(u16::MIN), None);
    assert_eq!(set.interval_containing_point(u16::MAX / 2), None);
    assert_eq!(set.interval_containing_point(u16::MAX), None);
}
...
fn handles_domain_edges() {
    ...
    assert_eq!(
        set.interval_containing_point(u16::MIN),
        Some(iv(u16::MIN, u16::MIN + 1))
    );
    assert_eq!(set.interval_containing_point(u16::MIN + 1), None);

    assert_eq!(
        set.interval_containing_point(u16::MAX - 1),
        Some(iv(u16::MAX - 1, u16::MAX))
    );
    assert_eq!(set.interval_containing_point(u16::MAX), None);
}
...
fn representative_points_match_contains_point_predicate() {
    ...
    for x in [u16::MIN, 9, 10, 19, 20, 30, 39, 40, 55, 60, u16::MAX] {
        assert_eq!(
