...
use crate::{
    U16COSetBuilder,
    u16::tests::test_support::{interval_pair, iv},
};

#[test]
fn seal_empty_builder_returns_empty_set() {
    let set = U16COSetBuilder::new().seal();
...
fn seal_handles_domain_endpoints() {
    ...
    b.insert(iv(u16::MIN, u16::MIN + 1));
    b.insert(iv(2, 3));
    b.insert(iv(u16::MAX - 1, u16::MAX));

    let set = b.seal();

    assert_eq!(
        set.as_slice(),
        &[iv(u16::MIN, u16::MIN + 1), iv(2, 3), iv(u16::MAX - 1, u16::MAX)]
    );
}
...
fn reference_normalize(mut xs: Vec<(u16, u16)>) -> Vec<U16CO> {
    xs.sort_unstable();

    let mut out: Vec<int_interval::U16CO> = Vec::new();
